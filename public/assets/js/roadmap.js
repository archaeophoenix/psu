class RoadMap {
    ...
    parseCoordinates(polyline) {
        if (!polyline) return null;

        let geom;
        try {
            geom = typeof polyline === "string" ? JSON.parse(polyline) : polyline;
        } catch (e) {
            console.warn("Gagal parse polyline:", polyline);
            return null;
        }
        ...
        if (type === "MultiLineString") {
            return coordinates.map(line => line.map(c => [c[1], c[0]]));
        }

        console.warn("Geometry tidak didukung:", type);
        return null;
    }

    renderAllPolylines() {
        if (!Array.isArray(this.roadData)) return;
    ...
    drawRoad(road) {
        ...
        polyline.on('click', () => {
            ...
            let photos = [];
            try {
                photos = JSON.parse(road.photo ?? "[]");
            } catch (e) {
                console.warn("Gagal parse photo:", road.photo);
            }
            ...
            photos.forEach((foto, index) => {
                ...
                $('#carouselInner').append(`
                    <div class="carousel-item ${activeItem}">
                        <img src="${baseUrl}/public/${foto}" class="d-block w-100" alt="Road Image ${index + 1}">
                    </div>
                `);
            });
            ...
            if (road.budget) {
                $('#budget').html(`
                    <iframe src="${baseUrl}/public/${road.budget}" class="img-fluid" style="max-height: 160px;"></iframe>
                `);
                $('#budget_download').attr('href', `${baseUrl}/public/${road.budget}`).show();
            }

            if (road.planning) {
                $('#planning').html(`
                    <iframe src="${baseUrl}/public/${road.planning}" class="img-fluid" style="max-height: 160px;"></iframe>
                `);
                $('#planning_download').attr('href', `${baseUrl}/public/${road.planning}`).show();
            }
            ...
            $('#detailMapLeft').html(`
                ...
                <p class="fs-4"><strong>Kondisi:</strong> <span class="badge bg-${badgeClass[road.condition] ?? 'secondary'}">${road.condition}</span></p>
                <p class="fs-4"><strong>Perkerasan:</strong> <span class="badge bg-${badgePaving[road.paving] ?? 'secondary'}">${road.paving}</span></p>
                <p class="fs-4"><strong>Status:</strong> <span class="badge bg-${badgeStatus[road.status] ?? 'secondary'}">${road.status}</span></p>
            `);

            $('#detailMapRight').html(`
                <p class="fs-4"><strong>Kecamatan:</strong> ${road.district}</p>
                <p class="fs-4"><strong>Lokasi:</strong> ${road.location}</p>
                <p class="fs-4"><strong>Kepadatan:</strong> <span class="badge bg-${badgePopulation[road.population] ?? 'secondary'}">${road.population}</span></p>
        ...
        polyline.on('mouseover', function (e) { this.openPopup(e.latlng); });
    ...
    addLegend() {
        ...
        this.legend.onAdd = () => {
            ...
            kondisi.forEach(k => {
                ...
                div.innerHTML += `
                    <i style="background:${color}; width:14px; height:14px; display:inline-block; margin-right:6px; border-radius:2px;"></i>
                    <b style="color:#fff">${k}</b><br>
                `;
            });
            return div;
        };
    ...
    setupFilter() {
        ...
        form.addEventListener("submit", (e) => {
            ...
            this.roadData.forEach(road => {
                try {
                    const roadLocation = (road.village_id ?? '').toString().trim().toLowerCase();
